...
IMPLICIT NONE    
  ...
  !! Changed name from 'NEIGBOURNODES'

  TYPE MLPGCONN
    integer,allocatable :: i(:)
  END TYPE MLPGCONN

  TYPE(MLPGCONN),ALLOCATABLE:: NLINK(:)  
...
IMPLICIT NONE
  ...
  INTEGER,ALLOCATABLE:: LINKTAB(:),IVV(:)
  DOUBLEPRECISION,ALLOCATABLE::SKK(:)
  REAL(KIND=8),ALLOCATABLE,TARGET::PTCSR(:)
  REAL(KIND=8),ALLOCATABLE,TARGET::FBCSR(:),SKKCSR(:)
  INTEGER(KIND=4),ALLOCATABLE,TARGET::IVVCSR(:),JVVCSR(:)
  REAL(KIND=8),ALLOCATABLE::WORK(:,:) !!(NODEID(0),NTHREADS)
  LOGICAL(KIND=1),ALLOCATABLE::NWORK(:,:) !!(NODEID(0),NTHREADS)
...
MODULE MLPGKINE
IMPLICIT NONE     
  DOUBLEPRECISION,ALLOCATABLE::UX(:,:),UY(:,:),UZ(:,:)
  DOUBLEPRECISION,ALLOCATABLE::PPX(:,:),PPY(:,:),PPZ(:,:)
  DOUBLEPRECISION,ALLOCATABLE::ROU(:),R(:),CC(:),R0(:)
  !DOUBLEPRECISION,ALLOCATABLE::PLAMDA(:)
  !INTEGER,ALLOCATABLE::iffree(:,:)       
END MODULE MLPGKINE
!!--------------------------END MLPGKINE---------------------------!!
...
IMPLICIT NONE
  ...
  TYPE :: PROBETYP
    ...
    REAL(KIND=8),ALLOCATABLE::XYZ(:,:),DATA(:,:)    
  CONTAINS
    PROCEDURE :: INITPROBE
  END TYPE PROBETYP
...
IMPLICIT NONE
  ...
  TYPE :: FLUXPLANE
    ...
    REAL(KIND=8),ALLOCATABLE::XFS(:), YFS(:), ZFS(:)
    REAL(KIND=8),ALLOCATABLE::X(:), Y(:), Z(:), WEI(:)
    REAL(KIND=8),ALLOCATABLE::U(:), V(:), W(:), P(:)
    !! BOTTOM-LEFT AND TOP-RIGHT COORD
    REAL(KIND=8)::BL(3),TR(3)
    !! OUTWARD UNIT NORMAL
    REAL(KIND=8)::NORM(3)
  CONTAINS
...
IMPLICIT NONE
  ...
  TYPE:: VOLPLANE
    ...
    REAL(KIND=8)::BL(2), TR(2) !XY ONLY    
    REAL(KIND=8),ALLOCATABLE::XFS(:), YFS(:), ZFS(:), WEI(:)
  CONTAINS
