!!----------------------------- MLPGR -----------------------------!!
!!    1.0.0   ->  Moving output to folder
!!    1.7b.0  ->  I_WM 15 actual particle vel X only
!!    1.7b.2  ->  I_WM 15 actual particle vel X and Z
!!    1.8b.2  ->  I_WM 15 actual particle pres
!!    1.8b.3  ->  correcting pressure
!!    1.9b.4  ->  FNPT coupling for region
!!            ->  weight = 1-3s^2+2s^3
!!    1.10.1  ->  Indented the functions 
!!                GRADIENT_2P  GRADIENT_POM
!!                WEIGHTF_GRAD_P  WEIGHTF_GRAD_2P
!!                WEIGHTF_MATRIX_MLPG
!!    1.10.2  ->  Taking care of KK=0
!!    1.10.3  ->  Changing RIAV in GRADIENT_POM
!!    1.10.4  ->  Changing RIAV in GRADIENT_POM
!!            ->  Printing RIAV at NODEID=5
!!----------------------------- MLPGR -----------------------------!!
!!    Files
!!    601   -   FNPT file
!!    602   -   Wave probes
...
PROGRAM THREED_BREAKINGWAVE
  ...
  TYPE :: CPUTIMETYP
    INTEGER(KIND=8)::TI(10),SYSRATEINT
    REAL(KIND=8)::SYSRATE
  END TYPE CPUTIMETYP      
  ...
  REAL(KIND=8),ALLOCATABLE::FB(:),PTMP(:),P(:)
  REAL(KIND=8),ALLOCATABLE::UN(:,:),UM(:)
  !LOGICAL FL

  REAL(KIND=8)::TEMPR(10)
  REAL(KIND=8),ALLOCATABLE::PRESS_DR(:)
  REAL(KIND=8),ALLOCATABLE::CSUXT1(:,:),CSUYT1(:,:),CSUZT1(:,:)
  ...
  INTEGER(KIND=4)::FSNOD1,FSNOD2,ERRSOL,NFS
  INTEGER(KIND=4),ALLOCATABLE::DZI(:,:)
  INTEGER(KIND=4),ALLOCATABLE::ERRTMP(:), MMTOIM(:)
  
  REAL(KIND=8),ALLOCATABLE::BNDXY(:,:), MIRRXY(:,:)
  INTEGER(KIND=4),ALLOCATABLE::BNDFIX(:,:)
  INTEGER(KIND=4),ALLOCATABLE::BNDFS(:)
  REAL(KIND=8),ALLOCATABLE::DDR(:)
  REAL(KIND=8),ALLOCATABLE::XFS(:),YFS(:),ZFS(:),ZFSTMP(:)
  REAL(KIND=8)::DOMBL(3),DOMTR(3),DOMX(2),DOMY(2),DOMZ(2)
  REAL(KIND=8)::CYLX, CYLY, CYLR

  TYPE(FNPTCPLTYP)::FP
  TYPE(NODELINKTYP)::MLDOM,FSDOM,BOTDOM
  TYPE(DOMAINDATA)::IM,CM,MM
  TYPE(CPUTIMETYP)::CPUT
  TYPE(PROBETYP)::WP,PP
  TYPE(FLUXPLANE),TARGET:: FLUXIN, FLUXOT
  TYPE(FLUXPLANE),POINTER:: F_PT
  TYPE(VOLPLANE)::VOLPL

  LOGICAL,ALLOCATABLE::FSSRCH(:)
  LOGICAL::RESUMECHK=.FALSE.

  OPEN(UNIT=8, FILE="mlpgTerOut.rout")
  ...
  CALL paralution_init(NTHR)
  !CALL OMP_SET_NUM_THREADS(8);
           

  !THE FOLLOING IS PERFOMRED ONLY FOR NSTAGE=0 NOTED ON 22/07/2005
  !IF (NSTAGE.EQ.0) THEN     ! FOR START FROM T=0 ADDED ON 22/07/2005

  !GENERATE THE NODES
  CALL NODEGRID(MESHFILE, FSNOD1, FSNOD2,  &
  ...
  !NODEID(-1) = INCLUDING WATER AND THE INNER SOLID WALL PARTICLE
  !NODEID(-2) = WATER PARTICLES


  ! DO IX=1,NODEID(-2)
  !   IF(NODEID(IX).EQ.5) THEN 
  !     RNODE=IX
  !     NODEID(IX)=0
  !     WRITE(8,*)'RNODE',RNODE,NODEID(RNODE)
  !   ENDIF
  ! ENDDO
  ...
  DDR=DDL
  !CALL NUMDEN1(DDR,RNODE)   !!! FOR N0
  !CALL NUMDEN2   !!!THE DUMMY PARTICLES' EFFECTS ON THE INNER WALL PARTICLES 
  ...
  CALL MLPG_GET_ETA(BOTDOM, IK, IM%CX(IX:IY), IM%CY(IX:IY), &
    ...
    MM%Z0(1:NPOI1), ERRTMP(1:NPOI1), DDL, NLMAXN, 1)
  ! MM%Z0 IS LOCAL DEPTH
  WRITE(8,'(" [INF] LOCAL DEP ERRSUM", F15.6)') SUM(ERRTMP(1:NPOI1))        
  WRITE(8,*)
  !!---------------------------------------------------!!
  ...
  CALL ALLOCATEWORK(NODEID(0),NTHR)
  ...
  OPEN(WP%FILE, FILE='Export//WaveProbes.dat')    
  OPEN(PP%FILE, FILE='Export//PressProbes.dat')
  OPEN(604,FILE='Export//CylindForce.dat')
  ...
  OPEN(1616,FILE='Export//CylPoi.dat')
  OPEN(1617,FILE='Export//BotDryPoi.dat')
  ...
  DO ISTEP = STEP0, NSTEPS 
    ! ISTART=0 FOR RUN STARTING FROM T=0, 22/07/2005

    ! WRITE DISC AND THEN STOP 22/07/2005
    ...
    IF((ISTEP.GT.0).AND.(MOD(ISTEP,REMESHFREQ).EQ.0))THEN
      WRITE(8,'(" [REM]----REMESHING AT TIMESTEP  = ",I10,"----")') &
        ISTEP
      ...
      CALL MLPG_GET_ETA(FSDOM, NFS, XFS(1:NFS), YFS(1:NFS), &
        ...
        MM%ET(1:NPOI1), ERRTMP(1:NPOI1), DDL, NLMAXN, 1)
      ...
      CALL MLPG_GET_UP(MLDOM,I,CM%CX(1:I),CM%CY(1:I),CM%CZ(1:I), &
        ...
        MM%UX(1:J),MM%UY(1:J),MM%UZ(1:J),MM%PR(1:J),DDL,NLMAXN)
    ...
    !!$OMP PARALLEL DEFAULT(SHARED) PRIVATE(I,DU,DV,DW)         
    ...
    !!---------------------------------------------------!!
    ! JUDGE THE FREE SURFACE PARTICLE
    !	CALL JUDGEFREESURFACE(ISTEP,ptmp) 
    CALL GENERATEGHOST(DDR)
    ...
    CALL ASSEMATRIX_MLPG_SHA(LNODE,NPOI1,NODEID(-2:NPOI1), &
      ...
      COORZ(:,2),SNX,SNY,SNZ,NLMAXN,MBAS,I_WM)
  
    CALL FILL_MATRIX_SHA(NTHR,LNODE,NPOI1,NODEID(-2:NPOI1), &
      NWALLID,COORX(:,2),COORY(:,2), &
      COORZ(:,2),NLMAXN,MBAS,KW,R1,DDR,DT,FB)  


    ! CALL ASSEMATRIX_MLPG_POM(FB,UM,ISTEP,WMA)
    ! CALL ASSEMATRIX_MPS1(FB,UN(:,1),ISTEP,WMA) !SEPERATE THE WATER PARTICLE AND WALL PARTICLE
    ! CALL ASSEMATRIX_MPS2(FB,VLAMDA,ISTEP)
    CALL SYSTEM_CLOCK(CPUT%TI(6))
    WRITE(8,'(" [TIM] TOTAL TIME FILLING MATRIX IS ",F15.6)'), &
      1D0*(CPUT%TI(6)-CPUT%TI(5))/CPUT%SYSRATE
    !x---------------------------------------------------x!
      
    !REVISE THE COEFFICIENT MATRIX SK AND FB, MAKE ITERATION QUICKER
    !THE COEFFICIENT OF EACH LINE IS DIVIDED BY THE MAX OF THE LINE
    !24/07/08      
    !PPX(:,3) = FB
    !CALL MAXNODIV(FB,SHU,NODN)

    !!---------------------------------------------------!!
    CALL SYSTEM_CLOCK(CPUT%TI(5))
    !CHANGE THE PRESSURE INTO P=P+(Z-D)
    DO I=1,NODEID(0) 
    ...
    !IF((ISTEP.EQ.0).OR.(IFSI.EQ.0))THEN
      !CALL JUDGEPRESSURE(WMA,PTMP,P,ISTEP,DDR)
    ...
    ! ! Force Cylinder      
    ! NPOI1=NODEID(0)
    ! CALL FINDCYLINFORCE(LNODE,NPOI1,NODEID(-7:NPOI1),NWALLID, &
    !   COORZ(:,1),P,SNX,SNY,SNZ,TMPR1,TMPR2,TMPR3)      
    ! WRITE(604,'(4E20.8)')TOTAL_TIME,TMPR1,TMPR2,TMPR3
    ...
    DO I = 1,NODEID(0)
      IF(ALLOCATED(NLINK(I)%I)) DEALLOCATE(NLINK(I)%I,STAT = IER)
    END DO
    WRITE(8,'(" [INF] DEALLOCATION ",I10)') IER    
...
SUBROUTINE INPUT(LNODETMP, H0, DDL, SCALE, KW, MBAS, DT,  &
  ...
  OPEN (UNIT=15, FILE='mlpgrInput.dat', STATUS='OLD')
