...
IMPLICIT NONE
  ...
  TYPE :: FNPTCPLTYP
    ...
    INTEGER(KIND=4)::MLNP
    INTEGER(KIND=4),ALLOCATABLE::MLPOI(:)
    REAL(KIND=8),ALLOCATABLE::MLVAL(:,:)
    REAL(KIND=8),ALLOCATABLE::FNCO(:,:), FNVA(:,:), FNFS(:,:)
  CONTAINS
...
CONTAINS
  ...
  !!------------------------FNPT_INTERP------------------------!!
  SUBROUTINE FNPT_INTERP(FN_NN, FN_CO, FN_VA, MP_NP, & 
    MP_POI, MP_VA, DDR)
  USE COMMONMOD
  IMPLICIT NONE
  !include 'COMMON.F'

  INTEGER(KIND=4),INTENT(IN)::FN_NN,MP_NP,MP_POI(MP_NP)
  REAL(KIND=8),INTENT(INOUT)::MP_VA(MP_NP,7)
  REAL(KIND=8),INTENT(IN)::FN_CO(FN_NN,2),FN_VA(FN_NN,6)
  REAL(KIND=8),INTENT(IN)::DDR(NODEID(0))

  INTEGER(KIND=4)::I,J,K,L,MP_I
  INTEGER(KIND=4)::NE_MA(FN_NN),NE_NP
  ...
  DO MP_I=1,MP_NP
    ...
    DO I=1,FN_NN
      ...
      IF(TMPR3.LE.1D0)THEN
        ...
        ELSE
          NE_VA(NE_NP,2)=4D0/3D0-4D0*TMPR3+4D0*TMPR3**2 &
            - 4D0/3D0*TMPR3**3
          TMPR7=TMPR7+NE_VA(NE_NP,2)
        ENDIF
      ENDIF
